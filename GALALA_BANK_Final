...
def financial_menu(
    current_user,
):  # Combine each of the following functions for financial operations (Deposit, Withdraw, Transfer, Check Balance)
    while True:
        clear()
...
def manage_pin():  # Manage Pin
    user_change_pin = input("\nEnter the user's account number: ").strip()
    found = False
    for user in user_data:
        if user_change_pin == user[acc_number]:
            user_new_pin = input("\nEnter the new PIN: ").strip()
...
def view_all_users():
    for user in user_data:
        print(
            f"Account Number: {user[acc_number]}, Name: {user[name]}, Balance: {user[balance]}, Is Admin: {user[is_admin]}"
        )
